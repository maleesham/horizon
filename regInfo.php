...
$sql = "SELECT id, firstName, lastName, email, password FROM registration";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<br> Id: ". $row["id"]."<br>". " - Name: ". $row["firstName"]. " " . $row["lastName"] . "<br> " ." - Email: ". $row["email"] . " <br>" ." - Password: ". $row["password"] . "<br> ";				
    }
} else {
    echo "0 results";
}
